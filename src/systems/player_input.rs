...
/// Method to find and use an item
fn use_item(n: usize, ecs: &mut SubWorld, commands: &mut CommandBuffer) -> Point {
    ...
    let weapons: Vec<String> = <(&Name, &Carried, &Weapon)>::query()
        .iter(ecs)
        .filter(|(_, carried, _)| carried.0 == player_entity)
        .map(|(n, _, _)| n.0.clone())
        .collect();

    let item_entity = <(Entity, &Item, &Carried, &Name)>::query()
        .iter(ecs)
        .filter(|(_, _, carried, _)| carried.0 == player_entity)
        .filter(|(_, _, _, name)| !weapons.contains(&name.0))
        .enumerate()
